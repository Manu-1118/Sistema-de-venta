...
<main id="main" class="main admin main-admin menu-toggle">
    ...
    <form method="POST" class="formulario">
        <fieldset>
            <legend>Detalles Compra</legend>

            <label for="txtProveedor">Proveedor:</label>
            <select class="form-control" name="txtProveedor" placeholder="Nombre del cliente" id="txtProveedor">
                <option value="">-- Seleccione un Proveedor --</option> </select>
        ...
        <div class="alinear-derecha separar-margin">
            ...
            <a class="boton-rojo" href="compras.php"> <span>Cancelar</span>
            </a>

            <button type="submit" id="generarCompra" class="boton-azul">Agregar Compra</button>
...
<script>
    ...
    const proveedoresData = <?php echo $proveedoresJSON; ?>;
        ...
        if (inputValue.length > 0) {
            ...
            if (filtered.length > 0) {
                filtered.forEach(p => {
                    ...
                    // --- CAMBIO CLAVE AQUÍ: Usar p.precio_compra en la visualización ---
                    li.textContent = `${p.nombre} - ${p.descripcion || ''} - $${parseFloat(p.precio_compra).toFixed(2)}`;
                    li.style.padding = '5px';
                    li.style.cursor = 'pointer';
                    li.dataset.codigo_producto = p.codigo_producto;
                    // --- CAMBIO CLAVE AQUÍ: Asignar p.precio_compra al dataset.precio ---
                    li.dataset.precio = p.precio_compra;
                    ...
                    li.addEventListener('click', function() {
                        ...
                        productoInput.dataset.precio = this.dataset.precio; // Ya es precio_compra
    ...
    document.getElementById('agregarProducto').addEventListener('click', function(e) {
        ...
        // --- CAMBIO CLAVE AQUÍ: 'precio' ya contiene el precio_compra gracias al cambio anterior ---
        ...
        if (existingProductIndex > -1) {
            ...
            productosEnCompra.push({
                ...
                nombre,
                cantidad,
                // --- CAMBIO CLAVE AQUÍ: Asegurarse de que el precio unitario guardado sea el precio_compra ---
                precio_compra_unitario: precio, // Guardamos el precio_compra para enviarlo al servidor
        ...
        productoInput.dataset.precio = ''; // Limpiar el dataset.precio
    ...
    function renderProductosTable() {
        ...
        productosEnCompra.forEach((p, index) => {
            ...
            // Aquí puedes decidir si quieres mostrar el precio_compra_unitario en la tabla
            fila.innerHTML = `
            <td>${p.nombre}</td>
            <td>${p.cantidad}</td>
            <td>${p.precio_compra_unitario.toFixed(2)}</td> <td>${p.total.toFixed(2)}</td>
    ...
    document.addEventListener("DOMContentLoaded", function() {
        ...
        const proveedorSelect = document.getElementById('txtProveedor');
        if (proveedoresData && Array.isArray(proveedoresData)) {
            proveedoresData.forEach(proveedor => {
                const option = document.createElement('option');
                option.value = proveedor.codigo_RUC;
                option.textContent = `${proveedor.codigo_RUC} - ${proveedor.nombre} ${proveedor.apellido || ''} - ${proveedor.empresa || ''} - ${proveedor.telefono || ''}`;
                proveedorSelect.appendChild(option);
            });
        } else {
            console.error("proveedoresData no es un array válido o está vacío:", proveedoresData);
        }
    });

    // Enviar formulario
    document.querySelector('.formulario').addEventListener('submit', function(e) {
        const proveedorID = document.getElementById('txtProveedor').value;
        if (!proveedorID) {
            Swal.fire('Atención', 'Por favor selecciona un proveedor de la lista.', 'info');
            e.preventDefault();
            return;
        }
        ...
        const productosParaEnviar = productosEnCompra.map(p => ({
            codigo_producto: p.codigo_producto,
            cantidad: p.cantidad,
            precio_compra: p.precio_compra_unitario // Enviamos el precio_compra_unitario al servidor
        }));

        inputProductos.value = JSON.stringify(productosParaEnviar);
    });
</script>
