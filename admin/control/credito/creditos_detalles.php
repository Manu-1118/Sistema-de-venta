...
//escribir el query
$query_mostrar_detalle = "SELECT p.nombre, p.precio_unitario, dc.cantidad, p.precio_unitario*dc.cantidad as 'subtotal', c.total FROM Credito c join DetalleCredito dc on c.id = dc.id_credito join Producto p on dc.codigo_producto = p.codigo WHERE c.id = {$_GET['id']}";
    ...
    <div class="contenedor-productos">
        ...
        <div class="tabla-containe">
            <table class="tabla-productos">
                ...
                <tbody>
                    <?php $i = 1;
                    while ($credito = mysqli_fetch_assoc($resultado_mostrar_detalle)): ?>
                        <tr>
                            <td><?php echo $i ?></td>
                            <td><?php echo $credito['nombre']; ?></td>
                            <td><?php echo $credito['precio_unitario']; ?></td>
                            <td><?php echo $credito['cantidad']; ?></td>
                            <td><?php echo $credito['subtotal']; ?></td>
                        </tr>
                    <?php $i++;
                        $total += $credito['subtotal'];
                    endwhile; ?>
                </tbody>
            </table>
